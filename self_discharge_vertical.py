...
OCV_data = pd.read_excel(file_path[0], sheet_name=2, skiprows=0, index_col=[0,1], usecols=np.arange(5,22)).T #horizontal layout (original proposal)
# OCV_data = pd.read_excel(file_path[0], sheet_name=2, index_col=[0,1,2,3], header=[0,1]) #.T.reset_index(drop=True) #verlical layout (reviewed by Camille)
